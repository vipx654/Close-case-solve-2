...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('🍁 ʜᴏᴡ ᴛᴏ ᴜꜱᴇ 🍁', url="https://https://t.me/+hGXzk9LsjmowYzNl")
                  ]]
        ...
        return
    if message.from_user.id not in ADMINS and len(message.command) == 1:
        await message.reply_text(
            "<b>⚠️ ᴘᴍ ᴅɪꜱᴀʙʟᴇᴅ !\n\nᴘʟᴇᴀꜱᴇ ꜱᴇᴀʀᴄʜ ꜰᴏʀ ꜰɪʟᴇs ɪɴ ᴏᴜʀ ɢʀᴏᴜᴘ.</b>",
                ...
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            ...
                            [
                                InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=f'https://t.me/from_Admins_desk') #Don't change anything without contacting me @LazyDeveloperr
                            ]
                        ]
                    )
                )
    ...
    if message.from_user.id not in ADMINS and not await db.has_premium_access(message.from_user.id):
        if temp.SHORT.get(message.from_user.id) is None:
            await message.reply_text(
                "<b>⚠️ ᴘʟᴇᴀꜱᴇ ꜱᴇᴀʀᴄʜ ꜰᴏʀ ꜰɪʟᴇꜱ ɪɴ ᴏᴜʀ ɢʀᴏᴜᴘ ꜰɪʀꜱᴛ !\n\n📝 ʏᴏᴜ ᴄᴀɴɴᴏᴛ ᴜꜱᴇ ᴅɪʀᴇᴄᴛ ꜰɪʟᴇ ʟɪɴᴋꜱ ᴡɪᴛʜᴏᴜᴛ ꜱᴇᴀʀᴄʜɪɴɢ ɪɴ ɢʀᴏᴜᴘ.</b>",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("📝 ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ", url=f"https://t.me/MoviesLinkSearchBot2")
                ]]),
                parse_mode=enums.ParseMode.HTML
...
async def requests(bot, message):
    ...
    if success:
        try:
            ...
            buttons = [[InlineKeyboardButton('ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ', url=link.invite_link)]]
            if 'reported_post' in dir() and reported_post is not None:
                buttons.append([InlineKeyboardButton('ᴠɪᴇᴡ ʀᴇǫᴜᴇsᴛ', url=f"{reported_post.link}")])
        except Exception as e:
            logger.warning(f"request invite link failed: {e}")
            buttons = []
        await message.reply_text("<b>ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ! ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ꜰᴏʀ ꜱᴏᴍᴇ ᴛɪᴍᴇ.\n\nᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ ꜰɪʀsᴛ & ᴠɪᴇᴡ ʀᴇǫᴜᴇsᴛ.</b>", reply_markup=InlineKeyboardMarkup(buttons) if buttons else None)
...
async def shortlink(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and int(userid) not in ADMINS:
